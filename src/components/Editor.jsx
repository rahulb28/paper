...
import {
  Bold, Italic, Code, Link as LinkIcon, Sparkles,
  Wand2, AlignLeft, List, Loader2
} from 'lucide-react'
...
const SLASH_COMMANDS = [
  ...
  { id: 'ai',      icon: '✦',   label: 'AI Write…',   hint: 'Generate with AI',       action: null }, // handled specially
]

const AI_BUBBLE_ACTIONS = [
  { id: 'improve',   label: 'Improve',   icon: Wand2 },
  { id: 'shorten',   label: 'Shorter',   icon: AlignLeft },
  { id: 'bullet',    label: 'To-do list', icon: List },
...
// ── AI Bubble toolbar ──────────────────────────────────────────────────────────
function AIBubble({ editor, onAITransform, aiLoading }) {
  return (
    <div className="flex items-center gap-0.5 bg-white border border-slate-200 rounded-xl px-1.5 py-1.5 shadow-xl">
      {/* Formatting */}
      <button onClick={() => editor.chain().focus().toggleBold().run()}
        className={`p-1.5 rounded-lg transition-colors ${editor.isActive('bold') ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50'}`}>
        <Bold size={14} />
      </button>
      <button onClick={() => editor.chain().focus().toggleItalic().run()}
        className={`p-1.5 rounded-lg transition-colors ${editor.isActive('italic') ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50'}`}>
        <Italic size={14} />
      </button>
      <button onClick={() => editor.chain().focus().toggleCode().run()}
        className={`p-1.5 rounded-lg transition-colors ${editor.isActive('code') ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50'}`}>
        <Code size={14} />
      </button>
      ...
      {/* AI actions */}
      {aiLoading ? (
        <div className="flex items-center gap-1.5 px-2 text-violet-600 text-xs font-medium">
        ...
        AI_BUBBLE_ACTIONS.map(action => (
          <button
            key={action.id}
            onClick={() => onAITransform(action.id)}
...
export default function Editor({ content, onChange, placeholder = 'Start writing, or type / for commands…', readOnly = false, docContext = '' }) {
  ...
  // Sync external content changes (e.g. from Firestore)
  useEffect(() => {
  ...
  const handleSlashAI = useCallback(async (prompt) => {
    ...
    try {
      ...
      if (data.content) {
        // Insert as markdown-parsed content
  ...
  return (
    <div className="relative" onMouseDown={e => e.target.closest('.slash-menu') && e.stopPropagation()}>
      ...
      {!readOnly && (
        ...
        >
          <AIBubble editor={editor} onAITransform={handleBubbleAI} aiLoading={aiLoading} />
        </BubbleMenu>
      )}
